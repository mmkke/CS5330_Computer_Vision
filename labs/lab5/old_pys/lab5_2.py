...
def apply_unsharp_mask(image, amount=1.0, threshold=0):
    blurred = image
...
def main():
    ...
    ## Unsharp Mask
    img1_usharp = apply_unsharp_mask(img1_BL, threshold=50)
    plt.imshow(img1_usharp, cmap = 'gray', vmin=0, vmax=255)
    plt.title('Unsharp')
    plt.show()

    ## Laplacian 
    img1_lap = cv2.Laplacian(img1_usharp, ddepth=cv2.CV_8U, ksize=5)
    print(img1_lap)
    plt.imshow(img1_lap, cmap = 'gray')
    plt.title('Laplacian')
    plt.show()
    ...
    ## Canny
    img1_canny = cv2.Canny(img1_usharp, 1, 15)
    plt.imshow(img1_canny, cmap = 'gray', vmin=0, vmax=255)
    plt.title('Canny')
    plt.show()

...
def main():
    ...
    img1_blur = cv2.bilateralFilter(img1, d=15, sigmaColor=150, sigmaSpace=150)
    ...
    # threshold each color channel in grayscale
    # take max (maybe mean?) values from each channel's grayscale image
    adaptive_thresh = np.mean( np.array(
                            [ cv2.adaptiveThreshold(img1_blur[:,:, 0], 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                                 cv2.THRESH_BINARY_INV, 11, 2), 
                              cv2.adaptiveThreshold(img1_blur[:,:, 1], 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                                 cv2.THRESH_BINARY_INV, 11, 2), 
                              cv2.adaptiveThreshold(img1_blur[:,:, 2], 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                                 cv2.THRESH_BINARY_INV, 11, 2)]),
                              axis=0)
    ...
    mean = np.mean(adaptive_thresh)
    adaptive_tresh_denoise = np.where(adaptive_thresh <= mean, 0, adaptive_thresh)
    plt.imshow(adaptive_tresh_denoise, cmap='gray')
    plt.title("adaptive_tresh_denoise")
    plt.show()

    ## smooth with bilateral filter
    edges_blur = cv2.bilateralFilter(adaptive_tresh_denoise.astype(np.uint8), d=10, sigmaColor=100, sigmaSpace=100)
    ...
    plt.savefig('images/final_image_adaptiveThresholding.png')
    plt.show()
